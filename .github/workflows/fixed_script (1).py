...
# ==========================================
# ضع رابط سيرفر التحكم الخاص بك هنا (الرابط الذي أنشأته على Hugging Face)
CONTROL_SERVER_URL = "https://ainalsaker2-malafat-almashrooaa-kamela.static.hf.space"
...
def load_data():
    if os.path.exists(SESSIONS_PATH):
        try:
            with open(SESSIONS_PATH, "r", encoding="utf-8") as f: return json.load(f)
        except: return {}
    return {}
...
def normalize_target_date(raw_value):
    ...
    try: parsed = datetime.strptime(raw_value.strip(), "%Y-%m-%d").date()
    except ValueError: return tomorrow.isoformat(), True
    if parsed < tomorrow: return tomorrow.isoformat(), True
    return parsed.isoformat(), False

def random_dob(is_head=True):
    days_back = random.randint(7300, 18000) if is_head else random.randint(365, 18000)
...
async def wait_for_new_token_from_server(phone, old_token):
    ...
    # 1. إعطاء أمر للسيرفر بطلب OTP
    print(f"\n{Y}⚠️  الحساب {phone} فقد التوكن. جاري طلب OTP جديد عبر السيرفر...{RESET}")
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{CONTROL_SERVER_URL}/admin/booking/auth/request-otp", json={"phone": phone}, headers=headers) as resp:
                if resp.status == 200:
                    print(f"{C}✅ تم طلب الـ OTP بنجاح! السكربت الآن ينتظر وصول الكود...{RESET}")
                elif resp.status == 429:
                    print(f"{R}⏳ السيرفر الأصلي يفرض حظراً مؤقتاً (Rate Limit) على طلب الـ OTP. سننتظر ونحاول لاحقاً.{RESET}")
    except Exception as e:
        print(f"{R}❌ فشل الاتصال بالسيرفر لطلب OTP: {e}{RESET}")
        
    # 2. حلقة الانتظار (Polling) لتحديث التوكن
                    ...
                    if response.status == 200:
                        ...
                        for acc in data.get("accounts", []):
                            if acc.get("phone") == phone:
                                ...
                                # إذا ظهر توكن جديد في قاعدة بيانات السيرفر
                                if server_token and server_token != old_token and server_token != "will-be-auto-filled":
                                    print(f"\n{G}🚀 تم استلام التوكن الجديد من السيرفر بنجاح! جاري استئناف الهجوم...{RESET}\n")
                                    return server_token
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            for slot in slots:
                ...
                try:
                    async with session.post(f"{BASE_URL}/booking/api/booking", json=payload, headers=headers, timeout=10) as response:
                    ...
                    if response.status == 429:
                        ...
                        # السبات الذكي وطلب التوكن تلقائياً
...
async def main():
    sessions = load_data()
    if not sessions:
        print(f"{Y}لم يتم العثور على حسابات محفوظة! يرجى إضافة حساب من لوحة التحكم (Dashboard) أولاً.{RESET}")
        return
